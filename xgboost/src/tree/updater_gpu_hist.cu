 ...
 public:
  ...
  void Reset(HostDeviceVector<GradientPair>* dh_gpair, DMatrix* dmat, int64_t num_columns,
    ...
    auto const& info = dmat->Info();
     if (interaction_constraints.interaction_constraints_.size() > 0)
    {
    this->column_sampler.Init(interaction_constraints.interaction_constraints_.size() , info.feature_weights.HostVector(),1.0f, 1.0f, 0.00001f);
             }
    else{
  ...
  void EvaluateLeftRightSplits(GPUExpandEntry candidate, ObjInfo task, int left_nidx,
    ...
    GPUTrainingParam gpu_param(param);
    std::shared_ptr<HostDeviceVector<bst_feature_t>> left_sampled_features;
    std::shared_ptr<HostDeviceVector<bst_feature_t>> right_sampled_features;
          if (interaction_constraints.interaction_constraints_.size() > 0)
    {
      auto feat_set = column_sampler.GetFeatureSet(0);
             
             
             std::unordered_set<bst_feature_t> cons = interaction_constraints.interaction_constraints_[feat_set->HostVector()[0]];
	//std::vector<bst_feature_t> h_features_set(cons.size());

	
	//auto h_features_set = std::make_shared<HostDeviceVector<bst_feature_t>>();
	...
	left_sampled_features = p_new_features;
	right_sampled_features = p_new_features; 
	    left_sampled_features->SetDevice(device_id);
	}
    else{
        left_sampled_features = column_sampler.GetFeatureSet(tree.GetDepth(left_nidx));
        right_sampled_features = column_sampler.GetFeatureSet(tree.GetDepth(right_nidx));
            left_sampled_features->SetDevice(device_id);
    }      
