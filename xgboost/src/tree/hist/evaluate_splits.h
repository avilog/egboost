...
template <typename GradientSumT, typename ExpandEntry> class HistEvaluator {
  ...
  void EvaluateSplits(const common::HistCollection<GradientSumT> &hist,
      ...
      if ((interaction_constraints_.interaction_constraints_.size() > 0) && (param_.colsample_bytree<1))
    {
      auto feat_set = column_sampler_->GetFeatureSet(tree.GetDepth(nidx));
             ...
             std::unordered_set<bst_feature_t> cons = interaction_constraints_.interaction_constraints_[feat_set->HostVector()[0]];
	...
	std::copy(cons.begin(), cons.end(),
       ...
       //h_features_set.insert(h_features_set.end(), cons.begin(), cons.end());
 ...
 public:
  ...
  explicit HistEvaluator(TrainParam const &param, MetaInfo const &info, int32_t n_threads,
    ...
    if ((interaction_constraints_.interaction_constraints_.size() > 0) && (param_.colsample_bytree<1))
    {
    column_sampler_->Init(interaction_constraints_.interaction_constraints_.size() , info.feature_weights.HostVector(),1.0f, 1.0f, 0.00001f);
             }
    else{
