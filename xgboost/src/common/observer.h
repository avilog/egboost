...
class TrainingObserver {
 ...
 public:
  ...
  template <typename T>
  void Observe_allways(std::vector<T> const& h_vec, std::string name,
               size_t n = std::numeric_limits<std::size_t>::max()) const {
    OBSERVER_PRINT << "Procedure: " << name << OBSERVER_ENDL;
  ...
  template <typename T>
  void Observe_allways(HostDeviceVector<T> const& vec, std::string name,
               size_t n = std::numeric_limits<std::size_t>::max()) const {
    auto const& h_vec = vec.HostVector();
    this->Observe_allways(h_vec, name, n);
  }
  template <typename T>
