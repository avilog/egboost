 ...
 public:
  ...
  void UpdateOneIter(int iter, std::shared_ptr<DMatrix> train) override {
    ...
    obj_->GetGradient(predt.predictions, train->Info(), iter, &gpair_);
        TrainingObserver::Instance().Observe(gpair_, "GetGradient");
  ...
  void Predict(std::shared_ptr<DMatrix> data, bool output_margin,
    ...
    if (pred_contribs) {
      ...
      out_preds->SetDevice(generic_parameters_.gpu_id);
        auto train =
      static_cast<std::shared_ptr<DMatrix>>(data);
      obj_->GetGradient(prediction.predictions, train->Info(), layer_end, &gpair_);
      std::vector<float> grads;
      for (auto& gp : gpair_.ConstHostVector())
	{
		grads.push_back(gp.GetGrad());
	}
	out_preds->Resize(grads.size());
      out_preds->Copy(grads); 
    } else {
      auto local_cache = this->GetPredictionCache();
      auto& prediction = local_cache->Cache(data, generic_parameters_.gpu_id);
      this->PredictRaw(data.get(), &prediction, training, layer_begin, layer_end);
